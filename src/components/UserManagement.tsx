...
export const UserManagement: React.FC<UserManagementProps> = ({
  ...
  return (
    <div id="user-management-section" className="bg-white dark:bg-slate-900/70 border border-slate-200 dark:border-slate-800 rounded-2xl p-5 md:p-6 shadow-sm dark:shadow-xl transition-colors space-y-4">
      {/* Information Banner: Problem Source & User Filter Scope */}
      <div className="p-3.5 rounded-xl bg-blue-50/70 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-900/50 flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
        <div className="space-y-1">
          <div className="font-semibold text-blue-900 dark:text-blue-200 flex items-center gap-1.5">
            <ShieldCheck className="w-4 h-4 text-blue-600 dark:text-blue-400 shrink-0" />
            <span>Problemset Data Source: Complete Official Codeforces Problem Archive</span>
          </div>
          <p className="text-blue-800/80 dark:text-blue-300/80 leading-relaxed">
            The generator uses the <strong>entire official Codeforces Problemset API (11,000+ problems)</strong>. User handles are strictly optional exclusion filters—adding your handle checks your submissions to guarantee you never get a problem you've already solved.
          </p>
        </div>
        <div className="shrink-0 font-mono text-[11px] px-2.5 py-1 bg-white dark:bg-slate-900 border border-blue-200 dark:border-blue-800 rounded-lg text-blue-700 dark:text-blue-300 font-bold self-start sm:self-auto">
          {uniqueExcludedCount > 0 ? `${uniqueExcludedCount} Solved Excluded` : '0 Solved Excluded'}
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              placeholder="Enter your Codeforces handle (or multiple handles, comma-separated)"
              disabled={isAdding}
